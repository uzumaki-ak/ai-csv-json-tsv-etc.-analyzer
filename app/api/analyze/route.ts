...
export async function POST(request: NextRequest) {
  try {
    ...
    if (apiProvider === "backend" || !apiProvider) {
      ...
      if (EURON_API_KEY) {
        try {
          ...
          if (res.ok) {
            ...
            console.error("Euron API error:", res.status)
          }
        } catch (e) {
      ...
      // Fallback to Gemini if Euron failed
    ...
    const parsedData = parseJSONFromText(extractedText)

    if (!parsedData || !parsedData.type) {
      console.error("Failed to parse valid data:", extractedText.slice(0, 500))
      return NextResponse.json({ error: "Invalid response format from API" }, { status: 400 })
    }
